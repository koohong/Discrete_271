...
- Unfortunately, there are only a few simple cases where these parameter
estimates have `closed-form solutions`; i.e., we cannot generally write
out the parameter estimates in terms of the observed data like we could
for the single probability estimate $\pi$ in Section 1.1.2.
...
- `saturated model`, which has a different coefficent for each data point, 
leading to perfect prediction, a likelihood of one, and a log likelihood of zero.
